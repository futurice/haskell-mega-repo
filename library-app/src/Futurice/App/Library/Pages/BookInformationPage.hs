...
bookInformationPage (BookInformationResponse binfoid title isbn author publisher published cover infoLink books language category) ls es = page_ "Book details " (Nothing :: Maybe Nav) $ do
    fullRow_ $ do
        ...
        div_ [ class_ "large-10 columns"] $ do
            table_ $ do
                ...
                vertRow_ "Language" $ toHtml $ maybe "" show language
                vertRow_ "Category" $ toHtml $ maybe "" show category
                vertRow_ "Info link" $ a_ [ href_ infoLink ] $ toHtml $ truncLink
