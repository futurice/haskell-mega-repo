...
statsPage
    ...
    -> Bool
    -> HtmlPage "stats"
statsPage world authUser sortCriteria sortDescEmpl = checklistPage_ "Stats" authUser $ do
    ...
    row_ $ large_ 12 $ table_ $ do
        ...
        tbody_ $ for_ sortedTask' $ \(SortedTask task counterActive counterArchive counterBoth) -> tr_ $ do
            td_ $ taskLink task
            td_ $ do
                showPercentageHtml counterActive
            td_ $ do
...
sortByEmpl SortByActive  (SortedTask _ (Counter i j) _ _) = if j == 0 then 0 else fromIntegral i/ fromIntegral j
sortByEmpl SortByArchive (SortedTask _ _ (Counter i j) _) = if j == 0 then 0 else fromIntegral i/ fromIntegral j
sortByEmpl SortByBoth    (SortedTask _ _ _ (Counter i j)) = if j == 0 then 0 else fromIntegral i/ fromIntegral j
