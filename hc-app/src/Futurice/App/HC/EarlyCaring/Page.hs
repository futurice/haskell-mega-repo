...
calculateBalances
    ...
    -> ([Balance], [NonEmpty Balance])
calculateBalances today personioEmployees0 planmillData absences0 absenceTypes = (rawBalances, balances)
  where
...
earlyCaringCSV esecret isPeopleManagerFinland today personioEmployees0 planmillData absences0 absenceTypes =
  ...
  where
    ...
    (_, balances) = calculateBalances today personioEmployees0 planmillData absences0 absenceTypes
...
earlyCaringPage esecret today interval personioEmployees0 planmillData absences0 absenceTypes = page_ "Early caring report" (Just NavEarlyCaring) $ do
  ...
  where
    ...
    (_, notInPlanMill) = partitionMaybe checkInPlanMill personioEmployees where
        checkInPlanMill e = do
            login <- e ^. P.employeeLogin
            pm <- pmMap ^? ix login
            return (e, pm)

    (rawBalances, balances) = calculateBalances today personioEmployees0 planmillData absences0 absenceTypes
